        ...
        if (not backend::uses_fft_types<backend_tag>::value){
            if (std::is_same<backend_tag, backend::fftw_cos>::value or
                ...
                scale_factor /= 8.0;
                      ...
                      std::is_same<backend_tag, backend::fftw_cos1>::value) {
                scale_factor = 1.0 / (8.0 * (plan.fft_sizes[0] - 1) * (plan.fft_sizes[1] - 1) * (plan.fft_sizes[2] - 1));
            }else if (std::is_same<backend_tag, backend::fftw_sin1>::value or
                      std::is_same<backend_tag, backend::cufft_sin1>::value) {
                scale_factor = 1.0 / (8.0 * (plan.fft_sizes[0] + 1) * (plan.fft_sizes[1] + 1) * (plan.fft_sizes[2] + 1));
            }else if(std::is_same<backend_tag, backend::rocfft_cos1>::value or
